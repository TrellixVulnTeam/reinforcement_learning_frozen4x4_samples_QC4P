...
class MonteCarlo:
    ...
    def calculate_new_G(self, G, gamma, r):
        return gamma * G + r

    def add_G_to_returns(self, G, state, action):
    ...
    def monte_carlo(self, episodes_count=10000, gamma=0.8, epsilon=0.2):
        ...
        for _ in range(episodes_count):
            ...
            for t in reversed(range(len(ep_policy))):
                G = self.calculate_new_G(G, gamma, ep_policy[t][2])

                # check is S[t] and A[t] wasn't in previously policys in current episode
                if (ep_policy[t][0], ep_policy[t][1]) not in [(prev_policy[0], prev_policy[1]) for prev_policy in ep_policy[:t]]:

                    self.add_G_to_returns(G, ep_policy[t][0], ep_policy[t][1])
